...
public class Main{
    public static void main(String[] args) {
        ...
        for (int i = 0; i < values.length; i++) {
            String region = data[i + 1][0];
            int[] regionValues = values[i];
            System.out.println("Регион: " + region);
            System.out.println("Среднее значение: " + calculateMean(regionValues));
            System.out.println("Медиана: " + calculateMedian(regionValues));
            System.out.println("Мода: " + calculateMode(regionValues));
            int[] quartiles = calculateQuartiles(regionValues);
            int interquartileRange = calculateInterquartileRange(quartiles);
            double variance = calculateVariance(regionValues, calculateMean(regionValues));
            double standardDeviation = calculateStandardDeviation(variance);

            System.out.println("Первый квартиль: " + quartiles[0]);
            System.out.println("Третий квартиль: " + quartiles[1]);
            System.out.println("Межквартильный размах: " + interquartileRange);
            System.out.println("Дисперсия: " + variance);
            System.out.println("Стандартное отклонение: " + standardDeviation);
            System.out.println();
        }
        String[] regions = {"Республика Казахстан", "Акмолинская", "Актюбинская", "Алматинская", "Атырауская",
        ...
        for (int i = 1; i < regions.length; i++) {
            for (int j = i + 1; j < regions.length; j++) {
                double correlation = calculateCorrelation(values, i, j);
                System.out.println("Correlation between " + regions[i] + " and " + regions[j] + ": " + correlation);
            }
        }
        ...
        // Predict the value for the future or other input
        int xInput = 80_000; // Example input
    ...
    private static double[] linearRegression(int[] xValues, int[] yValues) {
        ...
        // Calculate the sum of the products (X - meanX) * (Y - meanY) and the sum of squares (X - meanX)^2
        double sumXY = 0;
        double sumX2 = 0;
        for (int i = 0; i < n; i++) {
    ...
    private static double calculateCorrelation(int[][] values, int column1, int column2) {
        double sumX = 0;
        double sumY = 0;
        double sumXY = 0;
        double sumX2 = 0;
        ...
        for (int i = 0; i < n; i++) {
            int x = values[i][column1];
            int y = values[i][column2];
            sumX += x;
            sumY += y;
            sumXY += x * y;
            sumX2 += x * x;
        ...
        double correlationNumerator = (n * sumXY) - (sumX * sumY);
        double correlationDenominator = Math.sqrt((n * sumX2 - sumX * sumX) * (n * sumY2 - sumY * sumY));

        if (correlationDenominator == 0) {
            return 0; // Return 0 if denominator is zero to avoid division by zero error
        } else {
            return correlationNumerator / correlationDenominator;
        }
    }
    ...
    private static double calculateMedian(int[] values) {
        ...
        if (values.length % 2 == 0) {
            return (double) (values[middleIndex - 1] + values[middleIndex]) / 2;
        } else {
            return values[middleIndex];
        }
    }

    private static int calculateMode(int[] values) {
    ...
    private static int[] calculateQuartiles(int[] values) {
        Arrays.sort(values);
        int n = values.length;
    ...
    private static int calculateInterquartileRange(int[] quartiles) {
